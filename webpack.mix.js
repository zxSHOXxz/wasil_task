...
(glob.sync(`resources/mix/vendors/**/*.js`) || []).forEach(file => {
    ...
    // build plugin's js
    mix.scripts([file], `public/assets/plugins/custom/${pluginName}/${pluginName}.bundle.js`);
});

// Build custom 3rd party plugins styles
(glob.sync(`resources/mix/vendors/**/*.scss`) || []).forEach(file => {
    const pluginName = path.basename(file, '.scss');
    mix.sass(file, `public/assets/plugins/custom/${pluginName}/${pluginName}.bundle.css`);
});
